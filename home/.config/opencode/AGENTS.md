...
5. For each finding, write out a short PR comment. Don't add it to the actual PR unless I tell you to, just output the comment. The comment should include the line number and suggested fix, and use simple, conversational, easy to read English that doesn't sound like LLM output.

Do not run any tests in code review, as that will be covered in CI. If I ask you to post the comments, use the gh cli tool to post all the comments to github in a single review.
