      ...
      config = lib.mkMerge [
        ...
        (lib.mkIf (eachMempool != { }) {
          virtualisation.oci-containers = {
            backend = "docker";
            containers = eachMempool |> lib.mapAttrsToList mempool_containers |> lib.mkMerge;
          };

          systemd.services = eachMempool |> lib.mapAttrs' mempool_network_service;

          systemd.tmpfiles.rules = eachMempool |> lib.mapAttrsToList mempool_tmpfiles |> lib.flatten;
        })
      ];
    };
}
