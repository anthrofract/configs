  ...
  flake.nixosModules.electrs =
    ...
    in
    {
      ...
      config = lib.mkMerge [
        ...
        (lib.mkIf (eachElectrs != { }) {
          environment.systemPackages = [ pkgs.electrs ];

          systemd.services = eachElectrs |> lib.mapAttrs' electrs_instance;

          systemd.tmpfiles.rules =
            eachElectrs
            |> lib.mapAttrsToList (name: cfg: "d '${cfg.db_dir}' 0770 ${cfg.user} ${cfg.group} - -");
        })
      ];
    };
}
